...
## [2026-09-14] graph | 补记：节点更名后的重建（对齐 git 历史）

- **背景**：`3b246bf`「重建知识图谱（节点更名后，语义关系不变）」此前只作为 maintain 条目的收尾校验被顺带提及，没有独立的图谱记录。现补记。
- **数据**：`3b246bf` —— 64 节点 / 496 边。与 `336972a` 的规模完全相同，差别仅在于节点名随 `raw/` 文件名与 source 页 slug 的更名而变化，**页面之间的语义关系未变**。
- **复核**：重跑 `tools/build_graph.py` 后产物与已提交版本逐字节一致，图谱与 wiki 内容同步。
