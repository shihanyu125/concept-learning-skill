...
**上下文是模型单次请求里能"看到"的全部内容**；**上下文窗口（context window）是这堆内容的总量上限**，以 token（词元）计量。装不下的部分会被截断，模型相当于"忘了"。

组成：**输入 token**（你给的）+ **输出 token**（模型生成的）+ 部分模型的**推理 token**（模型"思考"用的）。

## 核心机制

- **计量单位 token**：模型把文本切成的最小单位。粗略换算——英文 1 token ≈ 0.75 个单词（约 4 字符）；中文一个常用汉字约 1–2 个 token，取决于分词方式。
...
3. 窗口**满了会遗忘**：这是长任务里 Agent 表现下滑的常见原因。

## 使用边界（易混淆点）

- **上下文 ≠ 长期记忆**：上下文是"这一次对话当下能看到"的，用完即散；长期记忆跨对话持久保存。
- **上下文窗口 ≠ 输出长度**：窗口是"输入 + 输出 + 推理"的总和，不只是模型能写多长。
- 空间宝贵：应**精简地喂信息**，而不是一股脑全丢进去。
